...
class UWCCheckOrderStatus extends WP_Widget {


    function UWCCheckOrderStatus() {
		
		$id_base = 'uwcorderstatus';
		$widget_ops = array('classname' => 'uwcorderstatus', 'description' => __( 'Проверка статуса заказа.', 'uwcart-start') );
		$widget_control_ops = array('title' => 'Проверка статуса заказа', 'show' => 'all', 'slug' => '', 'slug_exept' => '', '_multiwidget' => '0');
		//$widget_control_ops = array();
		$this->WP_Widget($id_base, __('UWСart-S: Проверка статуса заказа', 'uwcorderstatus'), $widget_ops, $widget_control_ops);
    ...
    function widget($args, $instance) {
		...
		$checkorderstatusform = $before_widget.$before_title.$title.$after_title;
		
		$checkorderstatusform .= '<table align="left" class="uwc_checkorderstatusform" cellspacing="0" cellpadding="0">';
		$checkorderstatusform .= '<form method="post" name="uwc_checkorder_form" style="margin:0px; padding:0px;">';
		$checkorderstatusform .= '<tr align="left"><td colspan="2">ваш email или номер заказа</td></tr>';
		$checkorderstatusform .= '<tr>';
		$checkorderstatusform .= '<td width="67%"><input type="text" name="id" style="width:99%;" /></td>';
		$checkorderstatusform .= '<td><input class="uwc_checkorderstatus_button" type="button" value="Проверить" onclick="javascript:if(!document.uwc_checkorder_form.id.value){alert(\'Введите ваш email или номер заказа\');document.uwc_checkorder_form.id.focus();return false;} jQuery(\'#popup_basket\').toggleOrderStatus({osid:this.form.id});return false;" /></td>';
		$checkorderstatusform .= '</tr>';
		$checkorderstatusform .= '</form>';
		$checkorderstatusform .= '</table>';
		
		$checkorderstatusform .= $after_widget;
		
		// условия, при которых виджет выводится в сайдбар
			switch ($show) {
				// везде
				case "all": 
					echo $checkorderstatusform;		
				break;
				// только на главной странице
				case "home":
					if (is_home()) echo $checkorderstatusform;
				break;
				// в постах
				case "post":
					...
					if ($InPost) echo $checkorderstatusform;
				break;
				...
				case "post_in_category":
					...
					if ($InCategory) echo $checkorderstatusform;
				break;
				...
				case "page":
					...
					if ($InPage) echo $checkorderstatusform;
				break;
			}
    ...
    function form($instance) {				
        ...
        ?>	
			...
			<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php if (!$title){$title = 'Проверка статуса заказа';} echo $title; ?>" />
			</label>
			</p>
			<p>
